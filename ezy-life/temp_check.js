...
function doLogin() {
  ...
  if (user.toLowerCase() === CREDS.user.toLowerCase() && pass === CREDS.pass) {
    if (document.getElementById('remember-me').checked) {
      localStorage.setItem('ezy_saved_creds', btoa(JSON.stringify({u: document.getElementById('l-user').value, p: pass})));
    } else {
...
function setAMPM(ampm) {
  _isPM = (ampm === 'PM');
  document.getElementById('t-am-btn').classList.toggle('active', !_isPM);
  document.getElementById('t-pm-btn').classList.toggle('active', _isPM);
}
...
function buildNoteBodyHtml(body) {
  ...
  lines.slice(0,20).forEach(function(l) {
    ...
    } else if (l.strip && l.strip() || l.trim()) {
...
function setGCalView(v) {
  _gcalView = v;
  ['day','week','month'].forEach(function(vv){
    document.getElementById('gcal-vbtn-'+vv).classList.toggle('active', vv===v);
  });
  renderGCal();
}
function gcalToday() {
...
function renderNewSubList(){ var el=document.getElementById('new-sub-list'); el.innerHTML=_newSubs.length===0?'<div style="font-size:12px;color:var(--text3);padding:4px 0 6px">No sub-tasks yet.</div>':_newSubs.map(function(s,i){ return '<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--border)"><span style="font-size:13px;flex:1">&bull;  '+esc(s.title)+'</span><button class="btn btn-outline btn-sm" onclick="remNewSub('+i+')" style="padding:2px 6px;color:var(--red)">?</button></div>'; }).join(''); }
...
function updateNavButtons() {
  ...
  // Always hide HOME button - it's confusing on mobile
...
function loadData() {
  try {
    ...
    var g = localStorage.getItem('ezy_goals_v2');
    if (g) goals = JSON.parse(g);
    var n = localStorage.getItem('ezy_notes_v2');
    if (n) notes = JSON.parse(n);
    var cv = localStorage.getItem('ezy_categories_v2');
    if (cv) { categories = JSON.parse(cv); }
    var nc = localStorage.getItem('ezy_next_cat_id');
    if (nc) _nextCatId = parseInt(nc,10) || _nextCatId;
    categories.forEach(function(x){ if(x.id >= _nextCatId) _nextCatId = x.id + 1; });
    var t = localStorage.getItem('ezy_tasks_v2');
    if (t) _tasks = JSON.parse(t);
    var nt = localStorage.getItem('ezy_next_task_id');
    if (nt) _nextTaskId = parseInt(nt,10) || 1;
    var sc = localStorage.getItem('ezy_sub_id_ctr');
    if (sc) _subIdCtr = parseInt(sc,10) || 1;
    var veh = localStorage.getItem('ezy_vehicles_v1');
    if (veh) _vehicles = JSON.parse(veh);
    var bd = localStorage.getItem('ezy_bizdata_v1');
    if (bd) _bizData = JSON.parse(bd);
    var bgt = localStorage.getItem('ezy_budgets_v1');
    if (bgt) _budgets = JSON.parse(bgt);
    var cbk = localStorage.getItem('ezy_checkbooks_v1');
    if (cbk) _checkbooks = JSON.parse(cbk);
    var fin = localStorage.getItem('ezy_finaccounts_v1');
    if (fin) _finAccounts = JSON.parse(fin);
  } catch(e){}
}
