...
var sidebarGroups = [
...

  {
    ...
    items: [
      ...
      { id:'fin-personal', icon:'👤', label:'Personal',    finTab:'personal' },

      { id:'fin-ietc',     icon:'💵', label:'Iron Eagle',  finTab:'ietc' },

      { id:'fin-bn1',      icon:'🏠', label:'B&N #1',      finTab:'bn1' },

      { id:'fin-bn2',      icon:'🏠', label:'B&N #2',      finTab:'bn2' }
  ...
  sidebarGroups.forEach(function(grp, gi) {
    ...
    if (!collapsed) {
      ...
      grp.items.forEach(function(item) {
        ...
        if (item.externalUrl) {
          html += '<a class="nav-item" id="nav-'+item.id+'" href="'+item.externalUrl+'" target="'+(item.target||'_blank')+'" style="text-decoration:none;color:inherit"><span class="icon">'+item.icon+'</span>'+esc(item.label)+'</a>';
        } else {
          html += '<div class="nav-item" id="nav-'+item.id+'" onclick="showPage(\''+item.id+'\')"><span class="icon">'+item.icon+'</span>'+esc(item.label)+badge+'</div>';
        }
...
var _finCategories = []; // [{id, name, icon, color}]

var _currentFinCategoryId = null;
...
function loadSidebarFromStorage() {
  ...
  try {
    ...
    // Ensure financials items always have finTab (in case old storage lacked it)
    var _defFinTabs = { 'fin-personal':'personal', 'fin-ietc':'ietc', 'fin-bn1':'bn1', 'fin-bn2':'bn2' };
    var _finGrp = sidebarGroups.find(function(g){ return g.id === 'financials'; });
    if (_finGrp) {
      _finGrp.items.forEach(function(item){ if (!item.finTab && _defFinTabs[item.id]) item.finTab = _defFinTabs[item.id]; });
      // Find index of financials group and force it expanded (never start collapsed)
      var _finGi = sidebarGroups.indexOf(_finGrp);
      if (_finGi >= 0) _sgCollapsed[_finGi] = false;
    }
  ...
  sidebarGroups.forEach(function(grp, gi) {
    ...
    if (isHome) {
      ...
      if (grp.directNav) {
        ...
        html += '<div class="sg-grp-header" onclick="showPage(\''+grp.directNav+'\')" style="flex:1;">';
      ...
      if (!collapsed) {
        ...
        grp.items.forEach(function(item, ii) {
          ...
          var itemOnclick = item.finTab
            ? 'setFinTab(\'' + item.finTab + '\');showPage(\'fin-budgets\')'
            : 'showPage(\'' + item.id + '\')';

          html += '<div class="nav-item" id="nav-'+item.id+'" onclick="'+itemOnclick+'" draggable="true" ondragstart="sgItemDragStart(event,'+gi+','+ii+')" ondragover="sgItemDragOver(event,'+gi+','+ii+')" ondrop="sgItemDrop(event,'+gi+','+ii+')" ondragend="sgItemDragEnd()" style="display:flex;align-items:center;">' +
        ...
        if (grp.id === 'financials') {
          html += '<div class="nav-item" onclick="openAddFinAccountModal()" style="display:flex;align-items:center;color:var(--accent2);font-weight:700;">'
            + '<span style="width:18px;flex-shrink:0;margin-right:6px;text-align:center;font-size:16px;">+</span>'
            + '<span>Add Account</span></div>';
        }
...
function showPage(id) {
  ...
  if (id === 'fin-dashboard') { renderFinDashboard(); }
  if (id === 'fin-cat-view') { renderFinCatView(); }
  ...
  var map = {
    ...
    'fin-dashboard':   ['💵 Financials','Categories'],
    'fin-cat-view':    ['💵 Financials','Accounts in category'],
...
function homeBlockClick(e, gid){
  ...
  // Financials gets its own dashboard view
  if (gid === 'financials') { showPage('fin-dashboard'); return; }
...
// ── Financials migration: ensure _finCategories exists and all accounts have categoryId ──
function migrateFinCategories() {
  // Ensure default "Uncategorized" category always exists
  if (!Array.isArray(_finCategories)) _finCategories = [];
  var hasDefault = _finCategories.some(function(c){ return c.id === 'cat-default'; });
  if (!hasDefault) _finCategories.unshift({ id:'cat-default', name:'Uncategorized', icon:'📂', color:'var(--accent)' });

  // Collect all accounts across old tab structure + flatten them
  var allAccounts = [];
  var tabs = ['personal','ietc','bn1','bn2'];
  // Also collect from flat array if _finAccounts is already flat
  if (Array.isArray(_finAccounts)) {
    allAccounts = _finAccounts;
    _finAccounts = {}; // reset to object
  } else {
    tabs.forEach(function(tab) {
      (_finAccounts[tab]||[]).forEach(function(a) { allAccounts.push(a); });
    });
  }

  // Migrate all existing accounts into flat storage _finAccounts.all
  // Only migrate if there are accounts without categoryId
  var needsMigration = allAccounts.some(function(a){ return !a.categoryId; });
  if (needsMigration) {
    allAccounts.forEach(function(a) {
      if (!a.categoryId) a.categoryId = 'cat-default';
    });
    // Move all accounts into _finAccounts.all (flat storage)
    var existing = _finAccounts.all || [];
    var existingIds = existing.map(function(a){ return a.id; });
    allAccounts.forEach(function(a) {
      if (!existingIds.includes(a.id)) existing.push(a);
    });
    _finAccounts.all = existing;
    // Clear old tab keys
    tabs.forEach(function(tab) { delete _finAccounts[tab]; });
  }

  // Ensure _finAccounts.all exists
  if (!_finAccounts.all) _finAccounts.all = [];
  // Ensure all accounts have categoryId
  _finAccounts.all.forEach(function(a) {
    if (!a.categoryId) a.categoryId = 'cat-default';
  });
}
...
function saveData() {
  ...
  if (!_fbUid) {
    ...
    try {
      ...
      localStorage.setItem('ezy_fincategories_v1', JSON.stringify(_finCategories));
    } catch(e) {}
    return;
  }
...
function loadData() {
  ...
  if (!_fbUid) {
    ...
    try {
      ...
      var fincat = localStorage.getItem('ezy_fincategories_v1'); if (fincat) _finCategories = JSON.parse(fincat);
  ...
  return _fbDb.collection('users').doc(_fbUid)
    ...
    .then(function(doc) {
      if (doc.exists) {
        ...
        if (d.finCategories)  _finCategories  = d.finCategories;
        migrateFinCategories();
  ...
  if (!_finCategories || !_finCategories.length) {
    ...
    _finCategories.forEach(function(cat) {
      ...
      html += '<div class="fin-dash-card" draggable="true"'
        ...
        + ' ondragover="finCatDragOver(event)"'
        + ' ondrop="finCatDrop(event,\'' + cat.id + '\')"'
        + ' ondragend="finCatDragEnd(event)"'
        + ' style="position:relative;cursor:grab;">'
        + '<div style="position:absolute;top:8px;right:8px;font-size:13px;color:var(--text3);cursor:pointer;padding:4px;pointer-events:auto;z-index:2;" onclick="event.stopPropagation();editFinCategory(\'' + cat.id + '\')">&#9998;</div>'
        + '<div class="fin-dash-card-icon" style="pointer-events:none;">' + esc(cat.icon || '📂') + '</div>'
        + '<div class="fin-dash-card-name" style="pointer-events:none;">' + esc(cat.name) + '</div>'
        + '<div class="fin-dash-card-meta" style="pointer-events:none;">' + catAccounts.length + ' account' + (catAccounts.length !== 1 ? 's' : '') + '</div>'
        + '<div class="fin-dash-card-bal" style="pointer-events:none;color:' + balColor + ';">$' + Math.abs(totalBal).toFixed(2) + (totalBal < 0 ? ' <span style="font-size:11px;color:var(--red);">owed</span>' : '') + '</div>'
        + '</div>';
    });
    html += '</div>';
  }
...
function setFinCatAndAddAcct(catId) {
  _currentFinCategoryId = catId;
  openAddFinAccountModal();
}

function setFinCatAndAddAcct_nav(catId, acctId) {
  _currentFinCategoryId = catId;
  openFinAccount(acctId);
}
...
function finCatDrop(e, catId) {
  e.preventDefault();
  e.stopPropagation();
  if (!_finCatDragSrc || _finCatDragSrc === catId) { _finCatDragSrc = null; return; }
  var arr = Array.isArray(_finCategories) ? _finCategories : [];
  var fromIdx = -1, toIdx = -1;
  for (var i = 0; i < arr.length; i++) {
    if (arr[i].id === _finCatDragSrc) fromIdx = i;
    if (arr[i].id === catId) toIdx = i;
  }
  if (fromIdx < 0 || toIdx < 0) { _finCatDragSrc = null; return; }
...
function finCatDragEnd(e) {
  _finCatDragSrc = null;
  document.querySelectorAll('.fin-dash-card').forEach(function(el){ el.style.opacity = ''; });
}
...
function renderFinPage() {
  ...
  var accounts = (_finAccounts.all || []).filter(function(a){ return !_currentFinCategoryId || a.categoryId === _currentFinCategoryId; });
...
function deleteFinAccount(id) {
  ...
  _finAccounts.all = (_finAccounts.all || []).filter(function(a){ return a.id !== id; });
