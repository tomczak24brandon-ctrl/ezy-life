...
async function main() {
  console.log('Fetching clean original...');
  ...
  // 3. Fix doLogin - case insensitive email, no captcha
  patch(
  ...
  patch('refreshCaptcha();\n\n// ===== REMEMBER ME =====', '// ===== REMEMBER ME =====', 'captcha bare call');
  // Fix loadSaved IIFE - convert to normal fn + call, also lowercase email restore
  patch("(function loadSaved(){\n  var saved = localStorage.getItem('ezy_saved_creds');\n  if (saved) {\n    try {\n      var c = JSON.parse(atob(saved));\n      var u = document.getElementById('l-user'), p = document.getElementById('l-pass');\n      if (u && c.u) u.value = c.u;\n      if (p && c.p) p.value = c.p;\n      document.getElementById('remember-me').checked = true;\n    } catch(e){}\n  }\n})()",
    "function loadSaved(){\n  var saved = localStorage.getItem('ezy_saved_creds');\n  if (saved) {\n    try {\n      var c = JSON.parse(atob(saved));\n      var u = document.getElementById('l-user'), p = document.getElementById('l-pass');\n      if (u && c.u) u.value = c.u.toLowerCase();\n      if (p && c.p) p.value = c.p;\n      document.getElementById('remember-me').checked = true;\n    } catch(e){}\n  }\n}\nloadSaved()", 'loadSaved IIFE->fn fix');
  patch("    refreshCaptcha();\n  }\n}\ndocument.addEventListener('keydown'", "  }\n}\ndocument.addEventListener('keydown'", 'captcha else');

  // 5. Error message
  patch('Incorrect email, password, or security answer.', 'Incorrect email or password.', 'error msg');

  // 6. No-cache meta
  patch('<meta charset="UTF-8">', '<meta charset="UTF-8"><meta http-equiv="Cache-Control" content="no-cache, no-store, must-revalidate"><meta http-equiv="Pragma" content="no-cache">', 'no-cache');
  ...
  if (tS !== -1) {
    var cS = html.indexOf('<div style="display:flex;gap:0;margin-bottom:20px', tS);
    var depth = 0, pos = cS;
  ...
  // 11. showPage closes sidebar
  patch('function showPage(id) {',
  ...
  while (true) {
    ...
    scripts.push({s:si+8,e:se,size:se-si}); sp=si+1;
  }
  ...
  if (dep.error) { console.error(dep.error); process.exit(1); }
