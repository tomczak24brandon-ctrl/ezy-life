...
async function main() {
  ...
  // Fix stale error message text
  html = html.split('Incorrect email, password, or security answer.').join('Incorrect email or password.');

  // Add a cache-bust meta tag so browsers don't cache the old broken version
  html = html.split('<meta charset="UTF-8">').join('<meta charset="UTF-8">\n<meta http-equiv="Cache-Control" content="no-cache, no-store, must-revalidate">\n<meta http-equiv="Pragma" content="no-cache">\n<meta http-equiv="Expires" content="0">');

  console.log('✅ Error message fixed + cache-bust headers added');
